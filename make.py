...
def get_app_uuid():
    ...
    if g_app_uuid == '':
        ...
        # The app may live at the repo root (legacy/in-development layout)
        # or under apps/<name>/ (current layout) - check both.
        app_config_file = None
        for candidate_dir in (g_app_name, os.path.join('apps', g_app_name)):
            candidate_file = os.path.join(candidate_dir, 'package.ini')
            if os.path.isdir(candidate_dir) and os.path.isfile(candidate_file):
                app_config_file = candidate_file
                break
